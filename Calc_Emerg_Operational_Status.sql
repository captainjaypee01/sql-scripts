...
BEGIN
  ...
  DECLARE cursor_i CURSOR FOR (SELECT count(*) As OfflineDevice, BuildingName FROM node_details 
  ...
  DELETE FROM `smartnew`.`node_operational_status` WHERE NodeType = 'EmergExitLight';
    INSERT INTO `smartnew`.`node_operational_status`
	(`BuildingName`,
	`MonitoredDevice`,
	`Operational`,
	`Alert`,
	...
	SELECT BuildingName, count(*) As MonitoredDevice, count(*) As Operational FROM node_details 
  ...
  read_loop: LOOP
    ...
    UPDATE `smartnew`.`node_operational_status`
