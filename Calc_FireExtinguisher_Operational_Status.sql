...
BEGIN
  ...
  DECLARE cursor_i CURSOR FOR SELECT count(*) As OfflineDevice, BuildingName FROM node_details as n
		...
		group by BuildingName, NodeType, NodeOnlineStatus, n.NetworkID having NodeType = 'FireExtinguisher' and NodeOnlineStatus = 0
  ...
  DELETE FROM `node_operational_status` WHERE NodeType = 'FireExtinguisher';
	...
	JOIN node_fx_logic fx on fx.NodeID = n.NodeID
	group by n.BuildingName, n.NodeType, n.NetworkID having NodeType = 'FireExtinguisher'
