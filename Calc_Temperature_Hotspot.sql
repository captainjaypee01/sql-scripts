...
DECLARE cursor_maximum CURSOR FOR SELECT n.BuildingName, n.SectorName, Max(SensorValue02) Maximum, Min(SensorValue02) Minimum FROM smartnew.node_latest_readings As reading 
JOIN smartnew.node_details As n on n.NodeID = reading.NodeID and n.NetworkID = reading.NetworkID
...
DECLARE cursor_current CURSOR FOR SELECT n.BuildingName, n.SectorName, Max(SensorValue02) Maximum FROM smartnew.node_latest_readings As reading 
JOIN smartnew.node_details As n on n.NodeID = reading.NodeID and n.NetworkID = reading.NetworkID
group by n.BuildingName,n.SectorName, SensorValue02, n.NodeType, reading.updated_at, n.NetworkID 
having n.NodeType = 'FireExtinguisher' and n.NetworkID in (SELECT NetworkID FROM smartnew.users_network where UserID COLLATE utf8mb4_general_ci  = userIDVal);

DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

TRUNCATE TABLE `smartnew`.`temperature_hotspot`;
INSERT INTO `smartnew`.`temperature_hotspot`
	(`Location`,
	`Sector`,
	`minimum`,
	`average`,
...
FROM smartnew.node_details group by BuildingName, SectorName, NodeType, NetworkID having NodeType = 'FireExtinguisher'
and NetworkID in (SELECT NetworkID FROM smartnew.users_network where UserID COLLATE utf8mb4_general_ci  = userIDVal);
...
OPEN cursor_maximum;
  read_maximum_loop: LOOP
    ...
    UPDATE `smartnew`.`temperature_hotspot`
