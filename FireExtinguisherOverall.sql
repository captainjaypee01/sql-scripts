...
BEGIN
    ...
    SET MonitoredDevices = (Select count(*) As MonitoredDevices from node_details where NodeType In ('FireExtinguisher','EmergLight','ExitLight') and NetworkID in (SELECT NetworkID FROM smartnew.users_network where UserID = userIDVal));
    SET OfflineDevices = (Select count(*) As OfflineDevices from node_details where NodeType In ('FireExtinguisher','EmergLight','ExitLight') and NodeOnlineStatus = 0 and NetworkID in (SELECT NetworkID FROM smartnew.users_network where UserID = userIDVal));
    ...
    on n.NodeID = alarm.NodeID and n.NetworkID = alarm.NetworkID and n.NodeType In ('EmergLight','ExitLight') and n.NetworkID in (SELECT NetworkID FROM smartnew.users_network where UserID = userIDVal) where alarm.IsResolved is null and n.NetworkID in (SELECT NetworkID FROM smartnew.users_network where UserID = userIDVal)
    ...
    on n.NodeID = alarm.NodeID and n.NetworkID = alarm.NetworkID and n.NodeType In ('FireExtinguisher','EmergLight','ExitLight') and n.NetworkID in (SELECT NetworkID FROM smartnew.users_network where UserID = userIDVal)
