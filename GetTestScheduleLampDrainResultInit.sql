...
BEGIN
    ...
    JOIN (SELECT nodeName, max(created_at) As max_created_at FROM (select * from exem_test_result where schedule = 1 and FIND_IN_SET (test_type, TestTypeList)) As maxup group by nodeName order by created_at desc) As lastOne
    ...
    #and adnode.is_schedule_responded = 1
