...
BEGIN
    ...
    JOIN (SELECT nodeName, max(created_at) As max_updated_at FROM (select * from smartnew.exem_test_result where schedule = 1 and test_type COLLATE utf8mb4_general_ci  in (TestTypeOne, TestTypeTwo)) As maxup group by nodeName) As lastOne
    ...
    and adnode.adhoclamp_status Not In ('0','5','6')
