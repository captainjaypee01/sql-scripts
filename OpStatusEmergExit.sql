...
BEGIN
	...
	SET OfflineDevices = (Select count(*) As OfflineDevices from node_details As n 
			where n.Status = 'Active' AND NodeType In (nodeTypeValue) and n.NodeOnlineStatus = 0  and n.NetworkID in (SELECT NetworkID FROM users_network where UserID = userIDVal));
